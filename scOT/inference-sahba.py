...
def get_trainer(
    ...
    # model_path2 = "camlab-ethz/Poseidon-L"
    # model2 = ScOT.from_pretrained(model_path2)
    # state2 = model2.state_dict()
    
    # for key in state1.keys():
    #     if key in state2:
    #         shape1 = state1[key].shape
    #         shape2 = state2[key].shape
    #         print(f"Layer: {key}")
    #         print(f"  Model 1 shape: {shape1}")
    #         print(f"  Model 2 shape: {shape2}")
    #         print(f"  Shapes match: {shape1 == shape2}\n")
    #         print(f"  Tensors match: {torch.equal(state1[key], state2[key])}\n")
    #     else:
    #         print(f"Layer {key} is missing in Model 2\n")
